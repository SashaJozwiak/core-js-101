...
function timeSpanToString(startDate, endDate) {
  ...
  const h = Math.trunc((difference / 3600000) % 100).toString().padStart(2, '0');
  const m = Math.trunc((difference / 60000) % 60).toString().padStart(2, '0');
  const s = Math.trunc((difference / 1000) % 60).toString().padStart(2, '0');
...
function angleBetweenClockHands(date) {
  ...
  const angle = 0.5 * (60 * hour - 11 * minutes);
