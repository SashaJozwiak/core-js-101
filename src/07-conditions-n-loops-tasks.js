...
function isInsideCircle(circle, point) {
  const hypotenuse = (((point.x - circle.center.x) ** 2) + ((point.y - circle.center.y) ** 2))
    ** (1 / 2);
...
function isCreditCardNumber(ccn) {
  return ccn.toString().split('').reverse().map((v, i) => {
    if ((i + 1) % 2 === 0) {
      const res = +v * 2;
      if (res > 9) return res.toString().split('').reduce((p, a) => +p + +a, 0);
      return res;
    }
    return v;
  })
    .reduce((p, a) => +p + +a, 0) % 10 === 0;
}
...
function getDigitalRoot(num) {
  const answer = num.toString().split('').reduce((p, a) => +p + +a, 0);
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    if (pathes.every((v) => v[i] === pathes[0][i])) result += pathes[0][i];
    else break;
  }
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < 3; i += 1) {
    if (position[i][0] === position[i][1]
      && position[i][1] === position[i][2]) if (position[i][0]) return position[i][0];
    if (position[0][i] === position[1][i]
      && position[1][i] === position[2][i]) if (position[0][i]) return position[0][i];
  }
